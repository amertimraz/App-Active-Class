...
- [X] T014 [US1] تنفيذ سيناريوهات 2، 3، 6، 7 من `quickstart.md` يدويًا (قفل بعد إعادة فتح التطبيق، قفل والتطبيق فاضل مفتوح، عدم تأثر انتهاء الترخيص الأساسي بيها، عدم تأثر باقي التطبيق)
  - **ملاحظة متأخرة (2026-08-30)**: T014 كانت متسيّبة [ ] من الجلسة الأصلية، ولمّا اتفحصت لاحقًا لقينا نسخة `booking_site/track/index.html` في هذا الـrepo **متأخرة عن النسخة المنشورة فعليًا على VPS** (اللي فيها فحص `parentPortalExpiresAt` سليم وشغّال) — يبدو إن التعديل اتعمل ورُفع للسيرفر في وقتها فعلاً (T010-T012) لكن مفيش commit وصل بيه لهذا الـrepo، فده سبب الالتباس اللي خلّى الميزة تبان "مش شغالة" رغم إنها شغالة فعليًا على الموقع. اتمت مزامنة `booking_site/track/index.html` مع النسخة الحية (diff صفر بعد التعديل)، **وكمان لقينا وأصلحنا باج تاني حقيقي غير موثّق هنا أصلاً**: `_watchLicenseChanges`/`_watchProfileChanges` (في `parent_portal_service.dart`) كانوا بيتسجّلوا بس أول مرة `publishProfile`/`pushStudentSummary`/`publishAllStudents` تتنادى فعليًا — يعني لو المدرس فتح التطبيق وماعملش أي إجراء بيلمس البوابة، وتغيّرت مدة/تفعيل البوابة (من الأدمن مثلاً)، مفيش أي إعادة نشر بتحصل للصفحة العامة لحد ما حاجة تانية تحصل بالصدفة تشغّل النشر لأول مرة. اتصلّحت بإضافة `ParentPortalService.init()` (public) بينادَى مرة واحدة في `main.dart` بعد تسجيل `LicenseController`/`SettingsController`، فالمستمعين بقوا مسجّلين من إقلاع التطبيق دايمًا بدل ما يتسجّلوا كسول. راجع `parent_portal_service.dart`/`main.dart` لتفاصيل الإصلاح.
