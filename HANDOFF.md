...
> آخر تحديث: 2026-09-19. المشروع: `C:\repo\active_class` — تطبيق Flutter عربي/RTL للمدرّسين الخصوصيين.
...
> **بعد الإصدار ده اتنفَّذ spec 038 (إسقاط مديونية)، spec 039 (تقسيم شاشات الطالب/المجموعة)، وspec 040 (تفاعل الطالب في حضور اليوم) بالكامل على الكود — flutter analyze/flutter test نضاف. DB version دلوقتي = 34 (spec 040). v1.2.68+4086 مبني ومتحقَّق منه جزئيًا على جهاز حقيقي (spec 038 فقط، لسه محتاج تأكيد نهائي من المستخدم)؛ spec 039 وspec 040 لسه محتاجين تحقّق يدوي كامل على جهاز.**

### 🔧 spec 040 — تفاعل الطالب في حضور اليوم (2026-09-19) — **الكود مكتمل، التحقق اليدوي معلَّق**
...
**⚠️ لسه محتاج قبل النشر**: تحقق يدوي كامل على جهاز حقيقي (`specs/040-student-interaction/quickstart.md`) — التسجيل الفردي، التطبيق الجماعي، ظهوره في سجل الطالب والـPDF (تأكد الألوان الثلاثة واضحة) ورسالة الواتساب، ووضع الفريق.

### 🔧 spec 039 — تقسيم شاشات إضافة/تعديل الطالب والمجموعة الضخمة (2026-09-19) — **الكود مكتمل، التحقق اليدوي معلَّق**
...
**⚠️ لسه محتاج قبل النشر**: تحقق يدوي كامل على جهاز حقيقي (10 خطوات في `specs/039-ui-forms-refactor/quickstart.md`) — الخطوة الأهم هي التأكد إن شيت تعديل المجموعة من شاشة "تفاصيل المجموعة" بقى فيه فعلًا اختيار أيقونة/لون/نوع تسعير (مش عيب، ده التغيير الموافَق عليه).

### 🔧 spec 038 — زرار إسقاط المديونية المتراكمة (2026-09-17) — **الكود مكتمل، التحقق اليدوي معلَّق**
...
كل التصحيحات اتبنت على نفس القاعدة الأصلية (`kDebtWriteOffNote`) وماغيّرتش سلوك "المتبقي/الحالة" (لسه بتتصفّر زي المتفق عليه) — بس زوّدت الاستبعاد من أي رقم "محصَّل فعليًا". v1.2.68+4086 مبني ومتثبّت على جهاز حقيقي، وموقَّع صح — المستخدم بيراجع النسخة دي حاليًا.
