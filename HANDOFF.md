...
> آخر تحديث: 2026-09-23. المشروع: `C:\repo\active_class` — تطبيق Flutter عربي/RTL للمدرّسين الخصوصيين.
...
> **آخر إصدار منشور رسميًا: v1.2.72+4090 — DB version = 34. منشور على GitHub Release (3 APKs: arm64/armeabi-v7a/x86_64) + مرفوع على VPS (arm64) + `.aab` مبني وجاهز لرفعه يدويًا على Google Play Console (لسه ماترفعش).**
> **specs 038 (إسقاط مديونية)، 039 (تقسيم شاشات الطالب/المجموعة)، وَ040 (تفاعل الطالب) اتحقّقوا فعليًا على جهاز حقيقي واتأكّدوا من المستخدم — شامل باجين اتكشفوا وانصلحوا أثناء التجربة الحية (استبعاد الإسقاط من كل أرقام "المحصَّل"، وطلاب مؤرشفون كانوا بيظهروا في تصحيح الامتحان/إرسال واتساب الجماعي). كل التغييرات دُفعت لـ`main` (commit 68ba213) ومنشورة رسميًا.**

### 🔧 spec 040 — تفاعل الطالب في حضور اليوم (2026-09-19) — **متحقَّق على جهاز حقيقي، منشور v1.2.72**

**الطلب:** مدرّس طلب تقييم بسيط لتفاعل الطالب في الحصة، معروض كإيموجي، يتسجّل وقت تسجيل الحضور، ويظهر في التقارير. 3 مستويات ثابتة: 😃 نشيط، 😐 عادي، 😴 غير متفاعل — اختياري، ومربوط بسجل حضور نفس اليوم (عمود جديد، مش كيان منفصل). المستخدم طلب كمان زرار "تطبيق على الكل" وإضافته لملخّص رسالة تقرير الشهر على واتساب.

**التنفيذ (`specs/040-student-interaction/`):**
- عمود `interaction` جديد على جدول `attendance` (محلي + Supabase) — DB version 33→34، migration `supabase/migration_student_interaction.sql` **مُطبَّقة فعليًا على السيرفر** (تأكدت ملكية الجدول `supabase_admin` قبل التنفيذ).
- `AttendanceController.setAttendanceStatus` بقت تمسح التفاعل تلقائيًا لما الطالب يبقى غايب؛ دالتين جديدتين `setInteraction`/`markGroupInteraction`.
- `attendance_page.dart`: صف 3 إيموجي تحت حالة الحضور (يظهر لحاضر/متأخر بس)، وزرار "تطبيق على الكل" جنب "تحضير الكل".
- **تصحيح تنفيذ مهم**: الخطة الأصلية كانت عرض الإيموجي نفسه جوه جدول PDF الشهري، لكن اكتُشف وقت التنفيذ إن خط PDF (Cairo TTF) ومكتبة `pdf` مش بيدعموا رسم إيموجي ملوّن — اتحل بتلوين النقطة الموجودة بالفعل بلون مخصَّص لكل مستوى (بنفسجي/أزرق/رمادي) بدل رسم الإيموجي، مع مفتاح ألوان نصي أسفل الجدول. رسالة الواتساب (نص عادي بيتعرض بخط الموبايل) معندهاش نفس المشكلة، فالإيموجي فيها زي ما هو.
- `monthly_report_message.dart`: سطر "🙋 التفاعل: 😃 X يوم • 😐 Y يوم • 😴 Z يوم" يستبعد أي مستوى صفر، ويختفي بالكامل لو مفيش أي تفاعل مسجَّل.
- `flutter analyze` = 30 (صفر جديد). `flutter test` = **193/193** ✅ (179 + 14 جديدة).

**✅ اتحقّق على جهاز حقيقي وأكّد المستخدم نجاحه** — بعد تحقّق إضافي المستخدم طلب إظهار الإيموجي جنب كل يوم في قائمة سجلات الحضور بالرسالة (مش بس ملخّص أعلى الرسالة) — اتنفّذ وتأكّد.

### 🔧 spec 039 — تقسيم شاشات إضافة/تعديل الطالب والمجموعة الضخمة (2026-09-19) — **متحقَّق على جهاز حقيقي، منشور v1.2.72**

**الطلب:** الملفات كانت كبيرة جدًا وبيصعب صيانتها: `add_student_sheet.dart` (1076 سطر)، `edit_student_sheet.dart` (848)، `groups_page.dart` (1467)، `group_details_page.dart` (2704). refactor بحت — بدون أي تغيير سلوك ظاهر للمستخدم، باستثناء واحد موافَق عليه صراحةً (تحت).

**اكتشاف مهم أثناء المراجعة**: فيه ازدواجية حقيقية بين `groups_page.dart` و`group_details_page.dart` — كل واحد عنده نسخته من شيت تعديل المجموعة ومحرر المواعيد. بمراجعة الكود الفعلي (مش افتراض)، تبيّن إن نسخة `groups_page.dart` (`_GroupFormSheet`) هي الأكمل (فيها اختيار أيقونة/لون/نوع تسعير + رسائل خطأ inline)، بينما نسخة `group_details_page.dart` أبسط. **باتفاق صريح مع المستخدم**، تم التوحيد نحو النسخة الأكمل — يعني شاشة "تفاصيل المجموعة" اكتسبت بعد كده تعديل أيقونة/لون/نوع تسعير ماكانتش موجودة فيها قبل كده (استثناء وحيد موثَّق من "صفر تغيير سلوك").

**التنفيذ (السبيك كامل في `specs/039-ui-forms-refactor/`):**
- `lib/widgets/student_form/` (جديد): `StudentDateButton`, `SiblingPicker` (دالة `showSiblingPicker`), `StudentCodeField` — وحدات مشتركة بين إضافة/تعديل الطالب، استبدلت 4 نسخ مكرَّرة.
- `lib/views/groups/group_form/` (جديد): `group_schedule_conflict.dart` (دوال صرفة موحَّدة + أول اختبار وحدة `test/group_schedule_conflict_test.dart`)، `group_schedule_editor.dart` (`GroupScheduleEditor`)، `group_form_sheet.dart` (`GroupFormSheet` — الشيت الموحَّد).
- `lib/views/groups/group_widgets.dart` وَ`group_details_widgets.dart` (جديدين): ويدجتس مساعدة صغيرة اتفصلت من الملفين الرئيسيين.
- **النتيجة**: `groups_page.dart` 1467→274 سطر (-81%)، `group_details_page.dart` 2704→1842 (-32%)، `add_student_sheet.dart` 1076→887، `edit_student_sheet.dart` 848→648.
- `flutter analyze` = 30 (تحسّن عن الأساس 34 — صفر جديد). `flutter test` = **179/179** ✅ (169 + 10 جديدة لدالة تعارض المواعيد).

**✅ اتحقّق على جهاز حقيقي وأكّد المستخدم نجاحه** — شامل شيت تعديل المجموعة من "تفاصيل المجموعة" اللي بقى فيه اختيار أيقونة/لون/نوع تسعير.

### 🔧 spec 038 — زرار إسقاط المديونية المتراكمة (2026-09-17) — **متحقَّق على جهاز حقيقي، منشور v1.2.72**

**الطلب:** زرار "إسقاط المديونية" في شاشة تفاصيل الطالب جنب كارت "مديونية متراكمة"، بنافذة تأكيد صريحة، يصفّر المديونية فورًا. **قرار مهم اتفق عليه المستخدم صراحةً**: المديونية تتصفّر 100% من كل مكان (زي أي دفعة عادية)، لكن في التقارير المُصدَّرة تفضل موسومة بوضوح "إسقاط مديونية" مش فلوس محصَّلة فعليًا.

**التنفيذ:** الإسقاط = صف `Payment` جديد بمبلغ يساوي `PricingHelper.accumulatedDebt` لحظة التأكيد الفعلي (مش وقت فتح النافذة)، بملاحظة ثابتة `kDebtWriteOffNote` — صفر تعديل بمخطط قاعدة البيانات، مزامنة وضع الفريق تشتغل مجانًا عبر `insertPayment` الموجودة. `report_controller.dart`/`dashboard_controller.dart`/`export_service.dart`/`payments_report_page.dart` كلهم بيستبعدوا الإسقاط من "الإجمالي المحصَّل" لكن بيسيبوا "المتبقي/الحالة" شاملة الإسقاط.

**النتيجة:** `flutter analyze` = 34 (صفر جديد وقتها). `flutter test` = 169/169 ✅.

**باجات اتكشفت واتصلّحت أثناء التجربة الحية على جهاز حقيقي (v1.2.68+4086)**: المستخدم لاحظ إن الإسقاط ظهر "بالظبط زي دفعة عادية" في 3 أماكن كان لسه ناقصهم استبعاد صريح للإسقاط من "الفلوس المحصَّلة فعليًا":
1. `student_details_page.dart` — "إجمالي المدفوعات" في هيدر شاشة تفاصيل الطالب كان بيجمع كل المدفوعات شاملة الإسقاط.
2. `dashboard_controller.dart` — كارت "دفعوا اليوم" في الصفحة الرئيسية (القائمة + العدّاد + الإجمالي) كان بيحسب كل الدفعات بصرف النظر عن الملاحظة — **قرار محسوم مع المستخدم**: الإسقاط يُستبعَد بالكامل هنا (مش بس من الإجمالي، من القائمة والعدّاد كمان — عكس سجل مدفوعات الطالب اللي فيه بيفضل ظاهر وموسوم).
3. `group_details_page.dart` (`paidAmount` — كاش محصَّل فعليًا شهريًا) وَ`payments_page.dart` ("إجمالي المحصّل" أعلى شاشة المدفوعات) كانا بيحسبوا الإسقاط ضمن الكاش المحصَّل.

كل التصحيحات اتبنت على نفس القاعدة الأصلية (`kDebtWriteOffNote`) وماغيّرتش سلوك "المتبقي/الحالة" (لسه بتتصفّر زي المتفق عليه) — بس زوّدت الاستبعاد من أي رقم "محصَّل فعليًا". **المستخدم أكّد نجاح كل التصحيحات على جهاز حقيقي.**

---

### 🐛 إصلاح: طلاب مؤرشفون يظهرون في تصحيح الامتحانات + إرسال واتساب الجماعي (2026-09-22/23)

**الطلب:** المستخدم لاحظ إن طالب مؤرشف ("ابراهيم هشام عبد العزيز") لسه بيظهر في شاشة تصحيح درجات امتحان جديد رغم أرشفته.

**السبب الجذري (اكتُشف على مرحلتين):**
1. **إصلاح أول ناقص**: `exam_grades_page.dart` كانت بتستخدم `DatabaseService.getStudentsByGroup()` لبناء خريطة أسماء الطلاب — فلترتها لتستبعد المؤرشفين (مع استثناء لو عنده درجة مسجَّلة بالفعل — سجل تاريخي)، لكن ده **مكنش كافي**.
2. **السبب الفعلي**: الصفوف المعروضة فعليًا في الشاشة مصدرها استعلام SQL منفصل تمامًا — `DatabaseService.getGradesForExamGroup()` — بيعمل `LEFT JOIN` من جدول `students` (بدون أي فلتر `is_archived`) مع `exam_grades`، فكل طالب في المجموعة بيطلع له صف (حتى لو مادرجش) بصرف النظر عن الأرشفة. ده المصدر الحقيقي اللي المستخدم شافه.

**الإصلاح النهائي**: إضافة شرط `AND (s.is_archived = 0 OR eg.id IS NOT NULL)` لاستعلام `getGradesForExamGroup` في `database_service.dart` — طالب مؤرشف بلا درجة مسجَّلة يختفي تمامًا، وطالب مؤرشف **عنده** درجة مسجَّلة بالفعل (اتأرشف بعد التصحيح) يفضل ظاهر كسجل تاريخي. نفس الفلترة اتطبّقت على الإرسال الجماعي بالواتساب من شاشة تفاصيل المجموعة (`_startGroupWhatsappBatchSend`) — دي معندهاش اعتبار "سجل تاريخي"، فالمؤرشف يتستبعد بالكامل.

**درس مهم**: لما شاشة فيها قائمتين مرتبطتين (خريطة أسماء + مصدر الصفوف المعروضة فعليًا)، لازم نتأكد إن الفلتر بيتطبّق على المصدر اللي بيبني الـUI فعليًا، مش بس على أي قائمة مساعدة بتبان متشابهة.

**النتيجة:** `flutter analyze` = 30 (صفر جديد). `flutter test` = 193/193 ✅. **المستخدم أكّد نجاح الإصلاح على جهاز حقيقي (v1.2.72).**
...
- **Google Play**: `.aab` مبني في `release_assets/v1.2.72/ActiveClass-play.aab` (66.4MB) — **لسه محتاج رفع يدوي من المستخدم على Play Console**.
- **Git**: كل تغييرات specs 038/039/040 + إصلاح الأرشفة اتعملهم commit واحد شامل (`68ba213`) ودُفعوا لـ`main`، وtag `v1.2.72` مدفوع على `origin`.
- DB version في هذا الإصدار = 34 (عمود `interaction` من spec 040) — migration `supabase/migration_student_interaction.sql` مُطبَّقة على السيرفر فعليًا.
