...
class PhoneHelper {
  ...
  ///  - بلا '+' → dialCode + الرقم بعد إزالة أصفار البداية.
  ...
  static String displayIntl(String raw, String dialCode) {
    ...
    if (dialCode == '20' && national.length == 10) {
      ...
      while (i < national.length) {
        final take = (national.length - i) > 4 ? 3 : (national.length - i);
        parts.add(national.substring(i, i + take));
        i += take;
      }
      grouped = parts.join(' ');
    }
  ...
  /// الجزء الوطني بين 7 و12 رقمًا وبلا حروف متبقّية.
