...
class _AddStudentSheetState extends State<_AddStudentSheet> {
  ...
  Future<void> _init() async {
    ...
    _selectedGroup =
        widget.preselectedGroup ?? (_groups.isNotEmpty ? _groups.first : null);
    await _refreshCode();
    _prefillPrice();
  ...
  Future<void> _submit() async {
    ...
    if (name.isEmpty) {
      _toast('أدخل اسم الطالب', isError: true);
      return;
    }
    if (g == null || g.id == null) {
      _toast('اختر المجموعة أولاً', isError: true);
      return;
    }
    if (_nextCode.isEmpty) {
      _toast(_codeIsManual ? 'أدخل كود الطالب' : 'لم يتم توليد الكود',
          isError: true);
      return;
    }
    if (price == null || price < 0) {
      _toast('أدخل رسوم صحيحة', isError: true);
      return;
    }
