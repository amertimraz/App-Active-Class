...
class _QRScannerPaymentPageState extends State<QRScannerPaymentPage> with WidgetsBindingObserver, SingleTickerProviderStateMixin {
  ...
  Student? _manuallySelectedStudent;
  String? _lastScan;
  ...
  void _selectStudent(Student student) {
    setState(() {
      _manuallySelectedStudent = student;
      searchResults = [];
  ...
  Widget _buildManualSearchTab(BuildContext context) {
    if (_manuallySelectedStudent != null) {
      return _buildPaymentDetails(context, _manuallySelectedStudent!);
    }
  ...
  Widget _buildPaymentDetails(BuildContext context, Student student) {
    return Obx(() {
      final months = controller.upcomingMonths;
      final selected = controller.selectedMonths;
      final total = controller.totalAmount.value;
            ...
            child: Column(
              ...
              children: [
                ...
                CustomButton(
                  label: 'تأكيد الدفع',
                  onPressed: () async {
                    // Set the student in controller for payment
                        ...
                        SnackBar(
                          content: Row(
                            children: [
                              ...
                              Text('تم تسجيل دفع: ${student.name}'),
                            ],
                          ),
  ...
  Future<void> _handle(String qr) async {
    ...
    if (_lastScan == qr && _lastScanAt != null && now.difference(_lastScanAt!).inMilliseconds < 1500) {
      return;
    }
    _lastScan = qr;
    _lastScanAt = now;
