...
class _PaymentsReportPageState extends State<PaymentsReportPage> {
  ...
  Widget _buildMonthlyReport() {
    return Obx(() {
        ...
        final Map<int, Group> groupById = {for (final g in groups) if (g.id != null) g.id!: g};
        ...
        final remainingAll = (totalDueAll - totalPaidAll).clamp(0.0, double.infinity).toDouble();
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                        ...
                        children: [
                          OutlinedButton.icon(
                            onPressed: () async {
                              ...
                              if (picked != null) {
                                paymentController.setMonth(DateTime(picked.year, picked.month, 1));
                              }
                            },
                      ...
                      _ProgressBar(value: progressAll, label: 'التقدم العام: ${FormatHelper.formatPercentage(progressAll * 100)}'),
                      ...
                      _SummaryStats(
                        ...
                        unpaidCount: (totalStudents - fullyPaidCountAll).clamp(0, totalStudents),
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      ...groups.map((g) {
                        final groupStudents = consideredStudents.where((s) => s.groupId == g.id).toList();
                        double groupDue = 0;
                        double groupPaid = 0;
                        ...
                        final groupRemaining = (groupDue - groupPaid).clamp(0.0, double.infinity).toDouble();
                        final groupProgress = groupDue > 0 ? (groupPaid / groupDue) : 0.0;
                        return Padding(
                          ...
                          child: _GroupRow(
                            ...
                            unpaidCount: (groupStudents.length - groupFullyPaid).clamp(0, groupStudents.length),
                            remaining: groupRemaining,
                            onTapUnpaid: () => _showUnpaidDialog(context, month, groupUnpaid, groupById, paidByStudent),
                          ),
                        );
                      }),
                    ],
              ...
              content: SizedBox(
                ...
                child: GridView.builder(
                  ...
                  itemBuilder: (context, index) {
                    final m = index + 1;
                    final label = DateFormat('MMMM', 'ar').format(DateTime(2000, m, 1));
                    return OutlinedButton(
                      onPressed: () => Navigator.of(context).pop(DateTime(year, m, 1)),
                      child: Text(label),
                    );
                  },
                ),
  ...
  Widget _buildDailyReport() {
    return Obx(() {
      ...
      final studentById = {for (final s in studentController.students) if (s.id != null) s.id!: s};
      final groupById = {for (final g in groupController.groups) if (g.id != null) g.id!: g};
              ...
              child: Padding(
                ...
                child: Wrap(
                  ...
                  children: [
                    ...
                    Chip(label: Text('إجمالي اليوم: ${FormatHelper.formatCurrency(total)}')),
                  ...
                  children: [
                    ...
                    if (payments.isEmpty)
                      const Padding(
                        ...
                        child: Center(child: Text('لا توجد مدفوعات في هذا اليوم')),
                      )
                    else
                      ListView.separated(
                        shrinkWrap: true,
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final group = student != null ? groupById[student.groupId] : null;
                          return ListTile(
...
class _SummaryStatsState extends State<_SummaryStats> {
  ...
  Widget _grid() {
    return Wrap(
      ...
      runSpacing: 8,
      children: [
        _miniStat('عدد الطلاب', widget.totalStudents.toString(), Icons.people_alt, Colors.blueGrey),
        _miniStat('المطلوب', FormatHelper.formatCurrency(widget.totalDue), Icons.request_page, Colors.orange),
        _miniStat('المدفوع', FormatHelper.formatCurrency(widget.totalPaid), Icons.attach_money, Colors.green),
        _miniStat('مدفوعين بالكامل', widget.fullyPaidCount.toString(), Icons.verified, Colors.teal),
        _miniStat('غير مدفوعين', widget.unpaidCount.toString(), Icons.pending, Colors.red, onTap: widget.onTapUnpaid),
        _miniStat('المتبقي', FormatHelper.formatCurrency(widget.remaining), Icons.account_balance_wallet, Colors.purple),
      ],
    );
  }

  Widget _miniStat(String title, String value, IconData icon, Color color, {VoidCallback? onTap}) {
                  ...
                  Row(
                    children: [
                      ...
                      Container(
                        ...
                        child: Text('${widget.studentCount} طالب', style: const TextStyle(fontSize: 11)),
                      ),
                    ],
                  ),
                  const SizedBox(height: 6),
                  LinearProgressIndicator(value: widget.progress.clamp(0.0, 1.0), minHeight: 6),
                  const SizedBox(height: 4),
                  Text('التقدم: ${FormatHelper.formatPercentage(widget.progress * 100)}', style: const TextStyle(fontSize: 11, color: Colors.grey)),
                ],
              ),
            ),
            IconButton(
        ...
        if (expanded) ...[
          ...
          Align(
            ...
            child: Wrap(
              ...
              runSpacing: 8,
              children: [
                _mini('المطلوب', FormatHelper.formatCurrency(widget.totalDue), Colors.orange),
                _mini('المدفوع', FormatHelper.formatCurrency(widget.totalPaid), Colors.green),
                _mini('المتبقي', FormatHelper.formatCurrency(widget.remaining), Colors.purple),
                _mini('مدفوعين بالكامل', widget.fullyPaidCount.toString(), Colors.teal),
                _mini('غير مدفوعين', widget.unpaidCount.toString(), Colors.red, onTap: widget.onTapUnpaid),
              ],
            ),
          ),
        ],
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 4),
                            Text('الكود: ${s.code} • المجموعة: ${g?.name ?? '-'}', style: const TextStyle(fontSize: 12, color: Colors.black54)),
                          ],
                        ),
                      ),
                      const SizedBox(width: 10),
                      Column(
                        ...
                        children: [
                          Text(FormatHelper.formatCurrency(remaining), style: const TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
                          _WhatsButton(student: s, groupName: g?.name ?? '-', monthLabel: monthLabel, remaining: remaining),
                        ],
                      ),
                    ],
                  ),
...
class _WhatsButton extends StatelessWidget {
  ...
  final double remaining;
  const _WhatsButton({required this.student, required this.groupName, required this.monthLabel, required this.remaining});
  ...
  Widget build(BuildContext context) {
    ...
    return IconButton(
      ...
      onPressed: !enabled ? null : () async {
        ...
        final msg = 'تذكير برسوم شهر $monthLabel للطالب ${student.name} (الكود: ${student.code}) في مجموعة $groupName. المتبقي: ${FormatHelper.formatCurrency(remaining)}';
        final uri = Uri.parse('https://wa.me/$phoneNorm?text=${Uri.encodeComponent(msg)}');
